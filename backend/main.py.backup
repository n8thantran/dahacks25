...
GITHUB_TOKEN = os.getenv('GITHUB_TOKEN')  # Required for automatic PR creation
...
async def analyze_codebase_security() -> dict:
    """
    ...
    files_to_scan = [
        "backend/main.py",
        "backend/templates/vulnerable_login.html"
    ]
    ...
    for file_path in files_to_scan:
        ...
        if full_path.exists():
            with open(full_path, 'r', encoding='utf-8') as f:
                ...
                codebase_content[file_path] = content
...
async def generate_vulnerability_fixes(codebase_analysis: dict) -> dict:
    """
...
- Fix DDoS: Add rate limiting middleware/decorator
...
async def create_github_pr(branch_name: str, summary: str, files_modified: list, base_branch: str = 'main') -> str:
    ...
    """
    if not GITHUB_TOKEN:
        logger.warning("GITHUB_TOKEN not found in environment. PR creation requires GitHub token.")
        raise ValueError("GITHUB_TOKEN environment variable is required for automatic PR creation")
...
**Generated automatically by ShieldOS AI agent.**

---
*This PR was created automatically after analyzing the codebase for security vulnerabilities.*
"""
...
async def handle_fix_command(chat_guid: str):
    ...
    if not GITHUB_TOKEN:
        send_text_message(chat_guid, "❌ GITHUB_TOKEN not found in .env file")
        send_text_message(chat_guid, "💡 Add GITHUB_TOKEN=your_token to .env")
        send_text_message(chat_guid, "🔗 Get token: https://github.com/settings/tokens")
        send_text_message(chat_guid, "📝 Required scopes: repo (full control of private repositories)")
        return
    ...
    codebase_analysis = await analyze_codebase_security()
    ...
    fixes = await generate_vulnerability_fixes(codebase_analysis)
    
    if not fixes:
        send_text_message(chat_guid, "❌ Failed to generate fixes. LLM service may be unavailable.")
        return
    
    # Get the repo root (assuming we're in backend/)
    repo_root = Path(__file__).parent.parent
    os.chdir(repo_root)
    
    # Set up git remote FIRST - use the repo from .env, NOT the current project's remote
    send_text_message(chat_guid, f"🔧 Configuring git remote to {GITHUB_REPO}...")
    repo_str = GITHUB_REPO.strip()
    ...
    subprocess.run(['git', 'remote', 'remove', 'origin'], check=False, capture_output=True, cwd=repo_root)
    subprocess.run(['git', 'remote', 'add', 'origin', remote_url], check=True, capture_output=True, cwd=repo_root)
